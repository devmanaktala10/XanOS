...
/*
 *  string_to_buffer
 *   DESCRIPTION: Copy the ASCII graphic of the input character into the buffer via the font
 *                data variable defined above. The graphic is 8 * 16 in size but the buffer
 *                holds 8 * 18 chunks so that 1 pixel above and below margin is left empty(0x00).
 *   INPUTS: buffer -- a pointer to an unsigned char array of fixed size to fill with ASCII graphic
 *           s -- a pointer to a char array containing status message
 *   OUTPUTS: none
 *   RETURN VALUE: none
 *   SIDE EFFECTS: writes ASCII graphic of status into buffer
 */
